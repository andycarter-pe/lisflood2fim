...
def fn_fix_diagonal_burns(str_dem_before_path, str_dem_after_path, str_dem_fixed_path):
    ...
    burned cardinal intermediary, a bridge pixel is inserted at one of the
    two shared cardinal positions. Its elevation is set to
    min(elev_A, elev_B) so it is always the lowest point on the crossing
    and will attract flow.

    Parameters
    ----------
    str_dem_before_path : str  - DEM before BurnStreamsAtRoads
