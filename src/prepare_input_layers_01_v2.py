...
def fn_fix_diagonal_burns(str_dem_before_path, str_dem_after_path, str_dem_fixed_path):
    ...
    Parameters
    ----------
    str_dem_before_path : str
        DEM before BurnStreamsAtRoads  (e.g. dem_breach_5070.tif)
    str_dem_after_path  : str
        DEM after  BurnStreamsAtRoads  (e.g. dem_burn_roads_5070.tif)
    str_dem_fixed_path  : str
        Output path for the D4-corrected DEM.

    Returns
    -------
    str_dem_fixed_path : str
    int_bridges : int
        Number of bridge pixels inserted.
    """
    import numpy as np
    import rasterio
    ...
    # ------------------------------------------------------------------
    # 2. Burned mask: pixels lowered by more than floating-point noise
    # ------------------------------------------------------------------
    burned = valid & (elev_after < elev_before - 1e-6)

    # ------------------------------------------------------------------
    # 3. Work on a mutable copy; iterate until no new bridges are needed
    #    (a single pass can leave new diagonal gaps where bridges land
    #     adjacent to the next burned cell in a run)
    # ------------------------------------------------------------------
        ...
        for r in range(height):
            for c in range(width):
                ...
                for dr, dc in diagonals:
                    ...
                    # diagonal neighbor must be in-bounds and burned
                    if not (0 <= nr < height and 0 <= nc < width):
                        continue
                    if not burned[nr, nc]:
                        continue

                    # The two shared cardinal intermediaries:
                    #   (r,  nc) -- same row as A, same col as B
                    #   (nr, c ) -- same row as B, same col as A
                    r1, c1 = r,  nc
                    r2, c2 = nr, c
                    ...
                    # If either cardinal intermediary is already burned,
                    # there is a D4 path — nothing to do for this pair.
                    if card1_burned or card2_burned:
                        continue

                    # Pick the first valid in-bounds cardinal as bridge site.
                    # Prefer (r, nc) — same row as A — so the bridge reads
                    # as "go east/west first, then north/south".
                    ...
                    if bridge_r is None:
                        continue   # both cardinal sites are nodata — skip

                    # Bridge elevation = lower of the two burned endpoints.
                    # Only ever lower the bridge cell, never raise it.
                    ...
                    if bridge_elev < elev_fixed[bridge_r, bridge_c]:
                        ...
                        burned[bridge_r, bridge_c] = True   # propagate
                        int_bridges += 1
                        changed = True   # re-scan needed

    # ------------------------------------------------------------------
    # 4. Write corrected DEM
    # ------------------------------------------------------------------
