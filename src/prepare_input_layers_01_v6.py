...
# ----------------------------
def fn_get_peak_intensity_pixel(polygon, str_cog_return_period):
    """
    Find the pixel with the highest intensity_by_return_period value
    that intersects a given polygon, using a COG GeoTIFF.

    Parameters
    ----------
    polygon : shapely.geometry.Polygon
        Catchment polygon in EPSG:4326.
    str_cog_return_period : str
        Path or URL to the intensity_by_return_period COG GeoTIFF.

    Returns
    -------
    dict with keys:
        lat, lon, row_idx, col_idx, intensity,
        return_period, return_period_band, n_pixels_in_polygon
    """
    da = rioxarray.open_rasterio(str_cog_return_period, masked=True)
    gdf_poly = gpd.GeoDataFrame(geometry=[polygon], crs="EPSG:4326")
    da_clipped = da.rio.clip(gdf_poly.geometry, gdf_poly.crs, drop=True)

    max_over_bands = da_clipped.max(dim='band', skipna=True)

    n_pixels = int((~np.isnan(max_over_bands.values)).sum())
    if n_pixels == 0:
        raise ValueError("No grid pixels found inside the polygon. "
                         "The polygon may be smaller than one grid cell.")

    flat_idx = np.nanargmax(max_over_bands.values)
    best_i, best_j = np.unravel_index(flat_idx, max_over_bands.shape)

    pixel_vals = da_clipped.values[:, best_i, best_j]
    best_band_local = int(np.nanargmax(pixel_vals))
    best_band_1idx  = best_band_local + 1

    with rasterio.open(str_cog_return_period) as src:
        tags = src.tags(best_band_1idx)
        rp_value = float(tags.get('return_period', best_band_1idx))

    return {
        'lat':                 float(da_clipped.y[best_i]),
        'lon':                 float(da_clipped.x[best_j]),
        'row_idx':             best_i,
        'col_idx':             best_j,
        'intensity':           float(pixel_vals[best_band_local]),
        'return_period':       rp_value,
        'return_period_band':  best_band_1idx,
...
# ----------------------
def fn_get_intensity_by_duration(peak_lat, peak_lon, str_cog_duration):
    """
    Return sorted list of intensity_by_duration values (in/hr)
    at the peak pixel location, sampled from the duration COG.
    """
    da = rioxarray.open_rasterio(str_cog_duration, masked=True)
    pixel = da.sel(x=peak_lon, y=peak_lat, method='nearest')
    intensities = [float(v) for v in pixel.values if not np.isnan(v)]
    return sorted(intensities)
# ----------------------


# ----------------------
def fn_get_intensity_by_return_period(peak_lat, peak_lon, str_cog_return_period):
    """
    Return sorted list of intensity_by_return_period values (in/hr)
    at the peak pixel location, sampled from the return period COG.
    """
    da = rioxarray.open_rasterio(str_cog_return_period, masked=True)
    pixel = da.sel(x=peak_lon, y=peak_lat, method='nearest')
    intensities = [float(v) for v in pixel.values if not np.isnan(v)]
...
def fn_fix_diagonal_burns(str_dem_before_path, str_dem_after_path, str_dem_fixed_path):
    ...
    For every pair of burned pixels that are diagonal neighbors with no
    burned cardinal intermediary, a bridge pixel is inserted at the lower
    of the two shared cardinal positions. Its elevation is set to
    min(elev_A, elev_B) so it is always the lowest point on the crossing
    and will attract flow.

    Parameters
    ----------
    str_dem_before_path : str  - Raw DEM before ANY conditioning (dem_clipped),
                                 used only to identify which pixels were burned.
                                 Must NOT be the breached DEM, because
                                 BreachDepressionsLeastCost may have already
                                 lowered road-crossing pixels, making the
                                 burned-pixel mask empty.
    str_dem_after_path  : str  - DEM after BurnStreamsAtRoads
    str_dem_fixed_path  : str  - Output path for the D4-corrected DEM.

    Returns
    -------
    str_dem_fixed_path : str
    int_bridges : int  - Number of bridge pixels inserted.
    """
    ...
    with rasterio.open(str_dem_after_path) as src:
        elev_after_raw = src.read(1).astype(np.float64)
        nodata_after   = src.nodata
        after_profile  = src.profile.copy()

    # Build valid mask from the burned DEM (it defines the active domain)
    if nodata_after is not None:
        valid = elev_after_raw != nodata_after
    else:
        valid = ~np.isnan(elev_after_raw)

    # Where before has nodata, also mark invalid
    if nodata is not None:
        valid &= (elev_before != nodata)
    else:
        valid &= ~np.isnan(elev_before)

    # Resize elev_before to match elev_after if grids differ (reproject mismatch)
    if elev_before.shape != elev_after_raw.shape:
        # Fall back: treat every valid after-pixel as potentially burned
        elev_before_matched = elev_after_raw + 1.0  # ensures all valid pixels pass
    else:
        elev_before_matched = elev_before

    # Burned = any pixel that BurnStreamsAtRoads lowered by more than a small
    # tolerance.  Use 0.01 m (not 1e-6) so that shallow burns on top of already-
    # breached terrain are still detected.
    burned = valid & (elev_after_raw < elev_before_matched - 0.01)

    elev_fixed  = elev_after_raw.copy()
    int_bridges = 0
        ...
        for r in range(height):
            for c in range(width):
                ...
                for dr, dc in diagonals:
                    ...
                    if not burned[nr, nc]:
                        continue
                    # Two candidate cardinal bridge positions
                    r1, c1 = r,  nc   # same row as A, same col as B
                    r2, c2 = nr, c    # same row as B, same col as A
                    ...
                    if card1_burned or card2_burned:
                        continue  # already connected cardinally
                    # Pick the lower candidate so fill_depressions won't erase it
                    bridge_r, bridge_c = None, None
                    best_elev_candidate = float('inf')
                    for br, bc in [(r1, c1), (r2, c2)]:
                        if 0 <= br < height and 0 <= bc < width and valid[br, bc]:
                            if elev_fixed[br, bc] < best_elev_candidate:
                                best_elev_candidate = elev_fixed[br, bc]
    ...
    # Write output using the burned-DEM profile (correct spatial ref / nodata)
    out_profile = after_profile.copy()
    out_profile.update(dtype=rasterio.float64, count=1)
    with rasterio.open(str_dem_fixed_path, "w", **out_profile) as dst:
...
def fn_condition_terrain(dict_filepaths, 
    ...
    # IMPORTANT: pass dem_clipped (the raw, pre-breach DEM) as the "before"
    # reference, NOT dem_breach.  BreachDepressionsLeastCost may have already
    # lowered the same road-crossing pixels, so comparing dem_burn_roads against
    # dem_breach gives elev_before ≈ elev_after at those cells and the burned
    # mask ends up empty -- no bridges get inserted.
    ...
    str_dem_burn_fixed, n_bridges = fn_fix_diagonal_burns(
        dict_filepaths['dem_clipped'],      # raw DEM before any conditioning
        dict_filepaths['dem_burn_roads'],   # DEM after BurnStreamsAtRoads
        str_dem_burn_fixed
    )
    # Overwrite the key so Steps 5 and 6 use the corrected DEM
...
# ----------------------
def fn_create_mannings_roughness_asc(
    str_url_nlcd_mannings,
    str_terrain_tif_path,
    str_out_folder_streams,
    str_out_folder_lisflood,
    b_print_output):
    """
    Reproject and resample an NLCD-derived Manning's n Cloud Optimized Geotiff
    onto the terrain grid and write an ASC for LISFLOOD-FP.

    The terrain raster defines the target CRS, resolution, and spatial extent —
    no separate polygon clipping is needed.

    Parameters
    ----------
    str_url_nlcd_mannings   : str  - path or URL to Manning's n COG (EPSG:5070)
    str_terrain_tif_path    : str  - path to conditioned terrain GeoTIFF
                                     (dem_burn_roads) — defines the target grid
    str_out_folder_streams  : str  - 01_stream_delineation folder
    str_out_folder_lisflood : str  - 02_lisflood_input folder
    b_print_output          : bool

    Returns
    -------
    str_mannings_asc_path : str - path to the written ASC file
    """

    if b_print_output:
        #print('  -- STEP N: Creating Manning\'s roughness layer')
        pass

    # --- Open terrain to get target grid properties ---
    dem = rioxarray.open_rasterio(str_terrain_tif_path, masked=True)

    target_crs        = dem.rio.crs
    target_resolution = dem.rio.resolution()
    target_bounds     = dem.rio.bounds()

    # --- Open NLCD Manning's n, clip to terrain bounds, reproject onto grid ---
    nlcd = rioxarray.open_rasterio(str_url_nlcd_mannings, masked=True)

    mannings = nlcd.rio.clip_box(*target_bounds) if nlcd.rio.crs == target_crs \
               else nlcd.rio.clip_box(*dem.rio.transform_bounds(nlcd.rio.crs))

    mannings = mannings.rio.reproject_match(
        dem,
        resampling=Resampling.average
    )

    # --- Set nodata to match LISFLOOD convention ---
    if mannings.rio.nodata is None:
        mannings.rio.write_nodata(-9999, inplace=True)

    # --- Derive output name from terrain filename ---
    str_terrain_stem   = Path(str_terrain_tif_path).stem   # e.g. 'dem_clipped_5070'
    str_mannings_name  = str_terrain_stem.replace('dem', 'manningsn') + '.asc'

    str_mannings_asc_path = os.path.join(str_out_folder_streams, str_mannings_name)

    mannings.rio.to_raster(str_mannings_asc_path, driver='AAIGrid')

    # --- Copy to 02_lisflood_input ---
    shutil.copy2(str_mannings_asc_path, Path(str_out_folder_lisflood) / str_mannings_name)

    if b_print_output:
        flt_min = float(mannings.min(skipna=True))
        flt_max = float(mannings.max(skipna=True))
        #print(f'     Manning\'s n range: {flt_min:.4f} – {flt_max:.4f}')
        #print(f'     Written: {str_mannings_name}')

    return str_mannings_asc_path
# ----------------------
...
""")
    ...
    global_section_schema = {
        'datasource': [
            ...
            'url_precip_intensity_by_duration',
            'url_nlcd_mannings'
        ],
        'lisflood_settings': [
            'downscale',
   ...
   # Create a boundary condition from the terrain file
    ...
    str_mannings_filename = Path(str_mannings_asc_path).name
    
    if b_print_output:
        print('  -- STEP 8: Determine bounding rain intensities')

    str_cog_rp  = dict_all_params['url_precip_intensity_by_return_period']
    str_cog_dur = dict_all_params['url_precip_intensity_by_duration']

    # --- Reproject catchment polygon to EPSG:4326 for COG lookup ---
    polygon_4326 = (
        gpd.GeoDataFrame(geometry=[polygon.geometry.iloc[0]], crs=polygon.crs)
        .to_crs('EPSG:4326')
        .geometry.iloc[0]
    )

    # --- Find the peak-intensity pixel inside the catchment polygon ---
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", RuntimeWarning)
        dict_peak_pixel = fn_get_peak_intensity_pixel(polygon_4326, str_cog_rp)

    if b_print_output:
        pass
        #print(f"     Peak pixel: lat={dict_peak_pixel['lat']:.4f}, "
        #      f"lon={dict_peak_pixel['lon']:.4f}, "
        #      f"intensity={dict_peak_pixel['intensity']:.3f} in/hr "
        #      f"(return period={dict_peak_pixel['return_period']} yr), "
        #      f"pixels in polygon={dict_peak_pixel['n_pixels_in_polygon']}")

    peak_lat = dict_peak_pixel['lat']
    peak_lon = dict_peak_pixel['lon']

    # --- Collect intensities from both COGs at the peak pixel ---
    list_intensity_by_duration_inhr = fn_get_intensity_by_duration(
        peak_lat, peak_lon, str_cog_dur)

    list_intensity_by_rp_inhr = fn_get_intensity_by_return_period(
        peak_lat, peak_lon, str_cog_rp)

    # --- Merge, deduplicate, sort, convert to mm/hr ---
    list_all_inhr = sorted(set(list_intensity_by_duration_inhr + list_intensity_by_rp_inhr))
    arr_intensity_mm_hr = np.array([round(x * 25.4, 1) for x in list_all_inhr])
    
    if b_print_output:
        print('  -- STEP 9: Writing paramter and rainfall data')
    ...
    for int_run_idx, flt_rain_mm in enumerate(arr_intensity_mm_hr):
        ...
        # Runs 1..default_manning_runs use uniform fpfric.
        # Runs after that use the spatially variable manningfile.
        # run_lisflood_02 will create the modified Manning's ASC after
        # the Nth run and patch the remaining .par files to point to it.
        b_default_manning_run = (int_run_idx < int_default_manning_runs)

        if b_default_manning_run:
            roughness_param = ("fpfric", dict_all_params['fpfric'])
        else:
           roughness_param = ("manningfile", str_mannings_filename)
