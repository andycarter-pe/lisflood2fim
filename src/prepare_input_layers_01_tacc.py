...
# Revised - 2026.03.03 -- WhiteboxTool Issues on TACC
# ************************************************************
...
def fn_condition_terrain(dict_filepaths, 
                         ...
                         str_url_roads):
    """
    Prepare DEM, stream network, and roads for Lisflood-FP.
    Works inside Apptainer/SIF on TACC.
    
    Parameters:
    -----------
    dict_filepaths : dict
        Paths for DEM, filled DEM, flow direction, flow accumulation, streams, vector streams, clipped roads, burned DEM.
        All paths must be under a writable directory (e.g., /work2/...).
    str_out_folder_streams : str
        Writable folder for all outputs (bound to container, e.g., /mnt).
    str_whitebox_path : str
        Full path to WhiteboxTools binary inside container (e.g., /opt/whitebox_tools/whitebox_tools)
    flt_threshold : float
        Flow accumulation threshold for stream extraction.
    polygon : shapely geometry
        Catchment polygon for road clipping.
    str_url_roads : str
        URL to roads dataset.
    """
    ...
    cmds_step2 = [
        # Fill depressions
        [str_whitebox_path, "--run=FillDepressions", f"--dem={dict_filepaths['dem_breach']}", f"--output={dict_filepaths['dem_filled']}"],
        # Flow direction
        [str_whitebox_path, "--run=D8Pointer", f"--dem={dict_filepaths['dem_filled']}", f"--output={dict_filepaths['dem_fdir']}"],
        # Flow accumulation
        [str_whitebox_path, "--run=D8FlowAccumulation", f"--dem={dict_filepaths['dem_filled']}", f"--out_type=cells", f"--output={dict_filepaths['dem_fa']}"]
    ]
    for cmd in cmds_step2:
        subprocess.run(cmd, check=True)

    # -----------------------------
    # Step 3 -- Extract Streams (Pass 01)
    cmd = [
        str_whitebox_path,
        "--run=ExtractStreams",
    ...
    cmd = [
        str_whitebox_path,
        "--run=RasterStreamsToVector",
        f"--raster={dict_filepaths['stream_raster']}",
        f"--d8_pntr={dict_filepaths['dem_fdir']}",
        f"--output={dict_filepaths['stream_vector']}"
    ]
    subprocess.run(cmd, check=True)
    ...
    # Get clipped roads
    gdf_roads_clipped = fn_get_clipped_roads(polygon, str_url_roads)
    gdf_roads_clipped.to_file(dict_filepaths['clipped_roads'], driver="ESRI Shapefile")
    ...
    cmds_step5 = [
        [str_whitebox_path, "--run=FillDepressions", f"--dem={dict_filepaths['dem_burn_roads']}", f"--output={dict_filepaths['dem_filled']}"],
        [str_whitebox_path, "--run=D8Pointer", f"--dem={dict_filepaths['dem_filled']}", f"--output={dict_filepaths['dem_fdir']}"],
        [str_whitebox_path, "--run=D8FlowAccumulation", f"--dem={dict_filepaths['dem_filled']}", f"--out_type=cells", f"--output={dict_filepaths['dem_fa']}"]
    ]
    for cmd in cmds_step5:
        subprocess.run(cmd, check=True)

    # -----------------------------
    # Step 6 -- Extract Streams (Pass 02)
    cmd = [
        str_whitebox_path,
        "--run=ExtractStreams",
        f"--flow_accum={dict_filepaths['dem_fa']}",
